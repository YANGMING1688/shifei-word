...
# -*- coding: utf-8 -*-
"""
OPC 网站导航栏更新 - 更新为截图中的样式
菜单：🏠 首页 | 👨 创始人 | 🏢 行上科技 | 📈 进化轨迹 | 📔 日记 | ✏️ 文章 | 🦀 关于 OPC
"""
...
def get_navbar_for_page(filename):
    ...
    # 清理其他占位符
    navbar = navbar.replace('{active_index}', '')
    navbar = navbar.replace('{active_founder}', '')
    navbar = navbar.replace('{active_company}', '')
    navbar = navbar.replace('{active_timeline}', '')
    navbar = navbar.replace('{active_diary}', '')
    navbar = navbar.replace('{active_articles}', '')
    navbar = navbar.replace('{active_about}', '')
...
def update_navbar_in_file(filepath, filename):
    ...
    # 匹配旧的导航栏结构
    pattern = r'<!-- 导航栏 -->\s*<nav class="navbar"[^>]*>.*?</nav>'
...
def main():
    ...
    print(f"🦞 OPC 网站导航栏更新")
    print(f"菜单：🏠 首页 | 👨 创始人 | 🏢 行上科技 | 📈 进化轨迹 | 📔 日记 | ✏️ 文章 | 🦀 关于 OPC")
    print(f"================================\n")
